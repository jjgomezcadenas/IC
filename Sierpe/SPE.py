...
###########################################################
class SPE:
    def __init__(self, pmt_gain=4.5e6, x_slope=5*units.ns, x_flat=1*units.ns):
        """
        Defines a spe
        """
    ...
    def SpePulse(self, t0, tmax=1e+6*units.ns):
        """
    ...
    def SpePulseTrain(self, tlow, tup, tmax=1e6*units.ns):
        ...
        if nmax >= NMAX:
            print "error in SpePulse train nmax = %d NMAX = %d" % (nmax, NMAX)
            sys.exit()
