...
class Filter:
    ...
    def __init__(self, ftype="high", fc=5E3, fs=1e+9):
        """
        Defines a Butterworth HPF (high pass frequency) or LPF
        (low pass frequencey) filter the default sampling frequencey is 1 GHz
        (inverse of 1 ns time) type may be equal to high or low
        """
        self.fc = fc
        self.fs = fs
        self.W = 2*self.fc/self.fs
        self.ftype = ftype
        self.b, self.a = SGN.butter(1, self.W, btype=self.ftype,
                                    analog=False, output="ba")
        self.ba, self.aa = SGN.butter(1, 2*self.fc, btype=self.ftype,
    ...
    def __str__(self):
        ...
        """.format(self.fc, self.fs, self.W, self.ftype)
        return s
...
class FEE:
    ...
    1) PMT gain G
    2) A HPF, due to the FEE decoupling capacitor C and the associated
       series resitor R
    3) A LPF that shapes the signal, with a frequencey f and a noise
    4) A resitor gain RG to transform current in voltage
    """

    def __init__(self, PMTG=4.5e6, C=6.75*units.nF, R=2350*units.ohm,
                 f=2E6*units.hertz, RG=250*units.ohm):
        self.PMTG = PMTG
        self.C = C
        self.R = R
        self.f_LPF = f
        self.RG = RG
        self.f_HPF = 1./(2*math.pi*R*C)
    ...
    def FEESignal(self, signal_current, noise_rms=0.3*units.mV):
        """
        Filters the input signal according to the filters and
        transforms it in volts
        """
    ...
    def DAQSignal(self, signal_t, signal_fee, noise_rms=0.3*units.mV):
        """
        Downscale the signal after the FEE
        """
        signal_t_d, signal_d = DownScaleSignal(signal_t, signal_fee,
                                               int(FP.time_DAQ))
    ...
    def __str__(self):
        ...
        """.format(self.PMTG, self.C/units.nF, self.R/units.ohm,
                   self.RG/units.ohm, self.f_HPF/units.hertz,
                   self.hpf.W, self.f_LPF/units.hertz, self.lpf.W)
        return s


def single_pe(start_pulse=100*units.ns, end_pulse=500*units.ns,
...
def pulse_train(signal_start=2000*units.ns, signal_length=5000*units.ns,
                daq_window=20*units.microsecond, noise=FP.NOISE_FEE_rms):
